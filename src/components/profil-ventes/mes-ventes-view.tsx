...
import { SALES, saleCount, type Sale, type SaleTab } from "@/data/mes-ventes";
...
export function MesVentesView() {
  const [tab, setTab] = useState<SaleTab>("en-cours");

  const filtered = useMemo(() => SALES.filter((s) => s.status === tab), [tab]);
    ...
    <li className="flex flex-wrap items-center gap-4 rounded-xl border border-gray-200 bg-white p-4 sm:flex-nowrap sm:gap-6">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Acheteur : <span className="text-gray-700">{item.buyer}</span>
        </p>
      </div>
